...
import { Menu, X, LogIn, LogOut, Crown } from "lucide-react";
    ...
    <header className="fixed top-0 left-0 right-0 z-50 glass-strong">
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        <Link to="/" className="shrink-0">
          <VitoriaLogo className="h-10 w-auto" />
        </Link>
            ...
            {user ? (
              <>
                <Button asChild variant="ghost" size="sm" className="text-gold">
                  <Link to="/assinantes"><Crown size={14} className="mr-1" />Área</Link>
                </Button>
                <Button onClick={signOut} variant="ghost" size="sm" aria-label="Sair">
                  <LogOut size={14} />
                </Button>
              </>
            ) : (
              <Button asChild size="sm" variant="outline">
                <Link to="/auth"><LogIn size={14} className="mr-1" />Entrar</Link>
              </Button>
            )}
          </div>
        </div>
